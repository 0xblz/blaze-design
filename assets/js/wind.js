...
// Combined JavaScript for Wind Vector 3D Wind Visualization
...
const StateCoordinates = {
  ...
  'Kansas': {
    center: [38.526600, -96.726486],
    zoom: 7
  },
  'Kentucky': {
    center: [37.668140, -84.670067],
    zoom: 7
  },
  'Louisiana': {
...
const WindCalculator = {
  ...
  calculateHurricaneEffect(x, y, z) {
    ...
    const distanceFromCenter = Utils.calculateDistance({
      x,
      z
  ...
  calculateThunderstormEffect(x, y, z) {
    ...
    const distanceFromStorm = Utils.calculateDistance({
      x,
      z
  ...
  getAltitudeWindPattern(flightLevel, x, z) {
    const patterns = [{
        max: 100,
        fn: () => Math.sin((x + z) * 0.6) * 25 + Math.cos(x * 0.8) * 15
      },
      {
        max: 200,
        fn: () => Math.sin((x - z) * 0.4) * 20 + Math.cos(z * 0.7) * 18
      },
      {
        max: 300,
        fn: () => Math.cos((x + z) * 0.3) * 15 + Math.sin(x * 0.5) * 12
      },
      {
        max: 400,
        fn: () => 30 + Math.sin(z * 0.2) * 10 + Math.cos(x * 0.3) * 8
      },
      {
        max: Infinity,
        fn: () => 45 + Math.cos((x - z) * 0.2) * 12 + Math.sin(x * 0.25) * 6
      }
    ];
...
const MapManager = {
  ...
  initLeaflet() {
    ...
    L.tileLayer(this.tileUrls[Config.settings.mapStyle], {
      maxZoom: 19
...
const EventHandlers = {
  init() {
    ...
    events.forEach(([event, handler]) => {
      const options = event.startsWith('touch') ? {
        passive: false
      } : undefined;
...
const Splitter = {
  ...
  init() {
    ...
    splitter.addEventListener('touchstart', this.startDrag.bind(this), {
      passive: false
    });
