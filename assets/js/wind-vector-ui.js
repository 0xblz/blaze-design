...
const Splitter = {
  ...
  drag(event) {
    ...
    if (this.isDragging) {
      this.handleMapDrag(deltaY);
    }

    if (this.isControlsDragging) {
      this.handleControlsDrag(deltaX, deltaY);
    }
  ...
  syncLayout() {
    ...
    if (this.isMobile()) {
      ...
      appContainer.style.setProperty('height', `calc(100vh - ${guiHeight}px - 12px)`, 'important');
    } else {
